...
# --- 1. 網頁基本設定 ---
st.set_page_config(page_title="中創園區空調聯防戰情室 V2.36", page_icon="❄️", layout="wide")
...
with st.sidebar:
    ...
    st.caption("去年 1~5 月無磁浮主機，需補償預估耗電：")
...
if is_holiday:
    # 假日真實負載校準：套用 160kW 實測基礎負載，空調相關加載全數歸零
    true_base_load = 160.0
    actual_load_growth = 0.0
    temp_penalty = 0.0
    shaved_kw_by_cap = 0.0
else:
    # 上班日邏輯：歷史基礎 + 冰機補償 + 擴編 + 高溫懲罰 - 磁浮封印
...
with c_action:
    if is_holiday:
        ...
        border_color = "#28a745" if suggested_ice_hrs <= 2 else "#ffc107" if suggested_ice_hrs <= 5 else "#dc3545"
...
with c_metrics:
    st.markdown(f"""
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px 15px; height: 100%; align-content: center;">
            ...
            <div>
                <div style="font-size: 15px; color: #555; margin-bottom: 4px;">目前短波輻射強度</div>
                <div style="font-size: 45px; font-weight: 700; color: #d35400; line-height: 1.1;">{current_rad} <span style="font-size: 20px; color: #555;">W/m²</span></div>
                <div style="display: inline-block; background: #f0f2f6; color: #666; padding: 2px 8px; border-radius: 10px; font-size: 13px; margin-top: 6px;">↑ 無視薄雲干擾之真實日照</div>
            </div>
            <div>
            ...
            <div>
                ...
                <div style="display: inline-block; background: #e6f4ea; color: #28a745; padding: 2px 8px; border-radius: 10px; font-size: 13px; margin-top: 6px;">{solar_ui_label}</div>
            </div>
        </div>
    """, unsafe_allow_html=True)
...
with sc1:
    ...
    st.markdown(f"""
        <div class="schedule-box">
            ...
            啟動：<span class="schedule-time" style="color:{time_color};">{start_time_str}</span><br>
            停止：<span class="schedule-time" style="color:{time_color};">{end_time_str}</span><br><br>
            <span style="font-size:16px; color:#666;">{memo_1}</span>
        </div>
    """, unsafe_allow_html=True)
with sc2:
    ...
    st.markdown(f"""
        <div class="schedule-box">
            ...
            啟動：<span class="schedule-time" style="color:{time_color};">{melt_start}</span><br>
            停止：<span class="schedule-time" style="color:{time_color};">{melt_end}</span><br><br>
            <span style="font-size:16px; color:#666;">{memo_2}</span>
        </div>
    """, unsafe_allow_html=True)
